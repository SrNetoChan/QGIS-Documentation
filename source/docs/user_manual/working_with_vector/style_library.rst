...
A set of tools is available to manage the symbol tree items and according to the level
selected, you'll get enabled different tools at the bottom of the dialog to:
...
Two kinds of symbol layers are used when editing a symbol in QGIS:

The symbol
...........

At the top level of the tree, it's of one of the **Marker**, **Line** and **Fill**
types. Each symbol can embed one or more symbols or symbol layers.
...
* :guilabel:`Size` and :guilabel:`Rotation` for marker symbol
* :guilabel:`Width` for line symbol
...
The symbols used at this level are items you can pick from the :ref:`symbols library
<vector_style_manager>`. A preview of same type symbols from the style manager dialog is
rendered in the dialog and can be filtered by selecting a group in the drop-down list
just above. Click the :guilabel:`Save` button to add the designed symbol to your symbol
library.
...
At a lower level, you can customize the symbol layers. While symbol layer types
differ according to the upper symbol type, they have some common parameters,
such as applying |paintEffects| :ref:`special effects <draw_effects>`.
